...
func TestHandleGmailLogin(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			url:        fmt.Sprintf("/gmail/login?jwt=%s", tokenString),
			statusCode: 307, // Redirect
		},
		{
			method:     http.MethodGet,
			url:        fmt.Sprintf("/gmail/login?jwt=%s", "invalid"),
	...
	for _, tt := range testData {
		...
		handler := http.HandlerFunc(HandleGmailLogin)
