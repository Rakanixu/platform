...
func TestHandleSlackLogin(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			url:        fmt.Sprintf("/slack/login?jwt=%s", tokenString),
			statusCode: 307, // Redirect
		},
		{
			method:     http.MethodGet,
			url:        fmt.Sprintf("/slack/login?jwt=%s", "invalid"),
	...
	for _, tt := range testData {
		...
		handler := http.HandlerFunc(HandleSlackLogin)
