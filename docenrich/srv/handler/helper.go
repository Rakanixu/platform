...
func retrieveDocFilesNotProcessed(ctx context.Context, c client.Client, dsID, index string) ([]string, error) {
	var ids []string
	from = 0
	size = 9999
...
func retrieveFiles(ctx context.Context, c client.Client, ids []string, index string, from, size int64) ([]string, error) {
	...
	for _, v := range r {
		if v.OptsKazoupFile == nil || v.OptsKazoupFile.ContentTimestamp.Before(v.Modified) {
			ids = append(ids, v.ID)
		}
	}
