...
var SignedURLOption *SignedURLOptions
...
func SignedObjectStorageURL(objName string) (string, error) {
	// Refresh when resource expires. 1 hour availability since was requested.
	SignedURLOption.Expires = time.Now().Add(3600000000000)

	url, err := SignedURL(globals.GC_THUMBNAIL_CONTAINER, objName, SignedURLOption)
	if err != nil {
		return "", err
	}
...
func UploadFile(file []byte, fId string) error {
	...
	_, err = srv.Buckets.Get(globals.GC_THUMBNAIL_CONTAINER).Do()
	if err != nil {
		return err
	}

	_, err = srv.Objects.Insert(globals.GC_THUMBNAIL_CONTAINER, &storage.Object{
		Name: fId,
